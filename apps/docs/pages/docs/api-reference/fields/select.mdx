...
Select fields are used to render a `select` input with a list of options. Extends [Base](base).
...
<ConfigPreview
  ...
  componentConfig={{
    ...
    render: ({ textAlign }) => {
      return <h1 style={{ textAlign, margin: 0 }}>Hello, world</h1>;
    },
  }}
/>
...
const config = {
  components: {
    Example: {
      ...
      render: ({ textAlign }) => {
        return <h1 style={{ textAlign }}>Hello, world</h1>;
      },
    },
  },
};
